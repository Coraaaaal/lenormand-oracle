/**
 * 灵境·雷诺曼 Edge Function v3
 * 路径: /edge-functions/oracle.js → URL: /oracle
 *
 * 本版本修复：
 *   🔴 #3  scenarios.bias 字段缺失 → 场景偏向现在正确注入 Prompt
 *   🟡 #4  rules condition 格式不匹配 → matchRule 现在真正能命中规则
 *   🟢 #7  time 字段未注入 Prompt → 时间维度现在出现在牌面标签描述中
 */

// ─────────────────────────────────────────────────────────────
// 1. 知识库（完整内联自 data/lenormand.json）
//    注意：这份 JS 内联版已同步修复了 JSON 里的两处格式问题：
//    - scenarios 每项加了 "bias" 字段
//    - rules condition 改为扁平列表格式（"dim:关键词"）
//    你的 data/lenormand.json 文件也需要替换为修复版（一同提供）。
// ─────────────────────────────────────────────────────────────
const LENORMAND_DB = {
  cards: {
  ...
  // rules condition 已改为扁平列表格式 "dim:关键词"，与 Flask match_rule 期望一致
  rules: [
    { id:"R1",  description:"信息类牌 + 变化类牌 → 变化被通知",       condition:{ card1_tags:["reality:消息","reality:信息","reality:文件"],           card2_tags:["dynamic:变化","dynamic:位移","dynamic:改善","dynamic:更迭"] }, output:"关于变化/迁移的通知/消息到来",        example:"骑士 + 鹳 = 调动通知" },
    { id:"R2",  description:"压力类牌 + 关系类牌 → 关系中的负担",     condition:{ card1_tags:["emotion:消极","emotion:困惑","emotion:沉重","emotion:侵蚀"], card2_tags:["reality:朋友","reality:信任","reality:家庭","reality:关系","reality:伴侣"] }, output:"在关系中存在消极的负担或误解", example:"云 + 狗 = 友谊出现误解" },
    { id:"R3",  description:"行动类牌 + 阻碍类牌 → 行动受阻",         condition:{ card1_tags:["dynamic:移动","dynamic:扩展","dynamic:触发","dynamic:行动"],  card2_tags:["dynamic:阻挡","dynamic:无法逾越","dynamic:停滞"] },              output:"行动或计划受到阻碍延误",              example:"船 + 山 = 行程延误" },
    { id:"R4",  description:"切割类牌 + 连结类牌 → 解除绑定",         condition:{ card1_tags:["reality:切割","reality:决断","reality:分离"],               card2_tags:["reality:承诺","reality:合同","reality:绑定","reality:连结"] },    output:"切断或终止了某段承诺/合同/关系",      example:"镰刀 + 戒指 = 离婚/解约" },
    { id:"R5",  description:"积极核心 + 终结类牌 → 好事终结",         condition:{ card1_tags:["emotion:积极","emotion:愉悦","emotion:热烈"],                 card2_tags:["reality:结束","reality:终结","reality:死亡"] },                  output:"积极美好的状态走向终结",              example:"花束 + 棺材 = 热情消退" },
    { id:"R6",  description:"静态稳固 + 消耗类牌 → 基础侵蚀",         condition:{ card1_tags:["dynamic:稳固","dynamic:固定","dynamic:停滞"],                 card2_tags:["dynamic:减少","dynamic:损坏","dynamic:内耗","dynamic:流失"] },   output:"稳定的基础正在被缓慢侵蚀消耗",        example:"房子 + 老鼠 = 房屋折旧" },
    { id:"R7",  description:"公开场合 + 隐藏物 → 隐匿",               condition:{ card1_tags:["reality:公众","reality:社交圈","reality:公开"],               card2_tags:["dynamic:隐藏","dynamic:未解锁","dynamic:秘密"] },                output:"在公开场合下存在隐藏的秘密或信息",    example:"花园 + 书 = 秘密社团" },
    // R8 人物牌规则需特殊逻辑，暂跳过
    { id:"R9",  description:"短暂性 + 长期性 → 中间状态",             condition:{ card1_tags:["time:短暂","time:转瞬","time:当下"],                         card2_tags:["time:长期","time:漫长","time:永久"] },                           output:"短暂的好转与长期状态之间的过渡",      example:"四叶草 + 百合 = 短暂的宁静" },
    { id:"R10", description:"情感类 + 阻碍类 → 情感阻力",             condition:{ card1_tags:["reality:爱情","reality:情感","reality:心"],                   card2_tags:["dynamic:阻挡","dynamic:压力","dynamic:无法逾越"] },             output:"情感遭遇阻碍或外部压力",              example:"心 + 山 = 情感阻碍" },
    { id:"R11", description:"变化类 + 根基类 → 生活/工作变动",        condition:{ card1_tags:["dynamic:位移","dynamic:改善","dynamic:更迭","dynamic:变化"],  card2_tags:["reality:家庭","reality:稳定","reality:根基","reality:机构"] },  output:"家庭/工作/居所发生变动或重组",        example:"鹳 + 房子 = 搬家/公司重组" },
    { id:"R12", description:"切割类 + 时间长期类 → 长期状态终结",     condition:{ card1_tags:["reality:切割","reality:决断"],                               card2_tags:["time:长期","time:持久","time:考验期"] },                         output:"长期存在的困境或负担被果断切断",      example:"镰刀 + 十字架 = 结束长期痛苦" },
  ],

  // scenarios 已加入 "bias" 字段（取自 bias_phrases[0]）
  scenarios: {
    love:  { name:"感情关系", bias:"从感情关系角度解读", domain_weight:{ love:1.5, work:0.4, daily:0.8 } },
    work:  { name:"事业工作", bias:"从职场/事业角度解读", domain_weight:{ work:1.5, love:0.3, daily:0.7 } },
    daily: { name:"日常综合", bias:"从日常生活角度解读", domain_weight:{ daily:1.2, love:0.8, work:0.8 } },
  },
};
...
// ─────────────────────────────────────────────────────────────
// 4. 规则匹配
//    🟡 修复#4：condition 现为扁平列表 "dim:关键词"，与 Flask 逻辑完全一致
// ─────────────────────────────────────────────────────────────
function matchRule(card1Tags, card2Tags) {
  for (const rule of LENORMAND_DB.rules) {
    const cond = rule.condition || {};
    const c1Keywords = cond.card1_tags || [];
    const c2Keywords = cond.card2_tags || [];
    if (!c1Keywords.length || !c2Keywords.length) continue;

    // 把两张牌的 reality/dynamic/emotion 拼成字符串
    const c1Text = [card1Tags.reality, card1Tags.dynamic, card1Tags.emotion, card1Tags.time].join(" ");
    const c2Text = [card2Tags.reality, card2Tags.dynamic, card2Tags.emotion, card2Tags.time].join(" ");

    // 关键词格式 "dim:词"，取冒号后的词做包含匹配
    function containsKeyword(text, kwList) {
      return kwList.some(kw => {
        const word = kw.includes(":") ? kw.split(":")[1] : kw;
        return text.includes(word);
      });
    }

    if (containsKeyword(c1Text, c1Keywords) && containsKeyword(c2Text, c2Keywords)) {
      return rule.output || "";
    }
  }
  return "";
...
// ─────────────────────────────────────────────────────────────
// 6. 构建 Prompt
//    🔴 修复#3：scenarioBias 现在读取 bias 字段，不再是空字符串
//    🟢 修复#7：time 字段出现在牌面描述中
// ─────────────────────────────────────────────────────────────
function buildPrompt(question, cards) {
  ...
  // 🔴 修复#3：直接读 bias 字段，始终有值
  const scenarioBias = scenarioData.bias || "通用视角";
  ...
  const cardLines = cardObjs.map(obj => {
    const t = obj.tags;
    return `第${obj.position}张【${obj.name}】→ 现实指向：${t.reality || "（缺失）"}；动态：${t.dynamic || "（缺失）"}；情绪：${t.emotion || "（缺失）"}；时间：${t.time || "（缺失）"}`;
  });

  // 🟡 修复#4：matchRule 现在真正能命中规则
  const ruleHints = [];
...
- 若牌有明确时间指向，可在解读末尾给出时间参考。
- 输出风格真诚清晰，避免空洞修辞。
- 不需要复述牌面标签，直接给出解读内容。`;
}

// ─────────────────────────────────────────────────────────────
// 7. 系统提示词（与 server-v2.py 完全一致）
// ─────────────────────────────────────────────────────────────
...
export async function onRequestPost(context) {
  ...
  if (!upstreamResp.ok) {
    ...
    return new Response(
      JSON.stringify({ error: `DashScope 错误 ${upstreamResp.status}: ${errText}` }),
      { status: 502, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } }
    );
  }

  // SSE 转换：DashScope → 前端期望的 {"text":"..."} 格式
